 ...
 * Падение процесса посреди задания откатывает и запуск {@code running}, и снятие с очереди —
 * задание остаётся в очереди с увеличенным {@code attempts}. Обработчик исполняется во вложенной
 * транзакции (savepoint): его ошибка БД не портит внешнюю, и запуск фиксируется как {@code failed}.
